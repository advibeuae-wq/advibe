...
- **`frontend-design` skill** — invoke it (`Skill: frontend-design`) before building or reshaping any web UI. It guides aesthetic direction, typography, and layout choices so output doesn't read as templated. Loading it is not optional for design work — treat it as step 2 of the working loop below, every time, not just on the first task of a session.
